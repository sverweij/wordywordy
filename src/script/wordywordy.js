const paramslikker = require("./utl/paramslikker");
const formatting = require("./utl/formatting");
const $ = require("./utl/browserutl");
const gaga = require("./utl/gaga");
const eventmap = require("./ui-control/eventmap");
const actions = require("./ui-control/actions");
const constants = require("./ui-control/constants");
...
function processParameters(pParams) {
  ...
  if (!pParams.text && pParams.canned) {
    $.ajax(
      decodeURIComponent(rCannedTexts[pParams.canned]),
      function (pEvent) {
  ...
  if (!pParams.text && pParams.url) {
    $.ajax(
      decodeURIComponent(pParams.url),
      function (pEvent) {
...
function retrieveStateFromLocalStorage() {
  if ($.localStorageOK()) {
    ...
    processLocalStorageKey(
      constants.LS_KEY_BUFFER,
      actions.initiateText,
      actions.getDocumentTitle()
    );
