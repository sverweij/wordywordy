...
function paste(pEvent) {
  if (pEvent && pEvent.clipboardData) {
    actions.initiateText(
      pEvent.clipboardData.getData("text/plain"),
      "clipboard"
    );
    actions.play();
  }
...
function percentageClick(pEvent) {
  actions.setPosFraction(pEvent.clientX / window.__percentagewrap.scrollWidth);
}
...
function drop(pEvent) {
  ...
  if (pEvent.dataTransfer.files.length > 0) {
    ...
    actions.initiateText(
      pEvent.dataTransfer.getData("text/plain"),
      "drag/ drop"
    );
    actions.play();
...
function addControlsIslandListeners() {
  window.__btn_home.addEventListener(
    "click",
    actions.gotoStartOfSentence,
    true
  );
  window.__btn_dec.addEventListener(
    "click",
    function () {
      actions.dec(false);
    },
  ...
  window.__btn_inc.addEventListener(
    "click",
    function () {
      actions.inc(false);
    },
  ...
  window.__btn_end.addEventListener(
    "click",
    actions.gotoStartOfNextSentence,
    true
  );
  window.__btn_paragraph.addEventListener(
    "click",
    actions.gotoStartOfNextParagraph,
    true
  );
}
function addSettingsIslandListeners() {
  ...
  window.__btn_fullscreen.addEventListener(
    "click",
    actions.toggleFullscreen,
    true
  );
  window.__btn_info.addEventListener("click", actions.toggleStatus, true);
  window.__lnk_more_information.addEventListener(
    "click",
    function (pEvent) {
...
function addMouseEventListeners() {
  ...
  window.__controls.addEventListener(
    "mouseover",
    actions.controlsMouseover,
    true
  );
  window.__controls.addEventListener(
    "mouseout",
    actions.controlsMouseout,
    true
  );
  window.__actionbar.addEventListener(
    "mouseover",
    actions.controlsMouseover,
    true
  );
